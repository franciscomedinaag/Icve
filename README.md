...
1. In `vite.config.ts` the `base` is set to `'/'` because this site will be served from the root of your custom domain `jrvolt.com`. If you later serve from a subpath (e.g. `username.github.io/repo`), change `base` to `'/repo/'`.
...
5. Wait a few minutes; your site will be available at `https://jrvolt.com/`.
...
1. Open your Cloudflare dashboard for `jrvolt.com` and add the following DNS records (set the proxy status to **DNS only** / grey cloud):
...
3. In GitHub repository Settings → Pages, set the custom domain to `jrvolt.com` (or push will add the CNAME automatically via the workflow). Enable "Enforce HTTPS" once GitHub provisions the certificate.
